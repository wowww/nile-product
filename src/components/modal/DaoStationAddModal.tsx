...
const DaoStationAddModal = ({ isOpen, setIsOpen, type, title, desc }: ModalProps) => {
      ...
      >
        ...
        <div className={cn('contents-block')}>
          <div className={cn('modal-header')}>
            <h3 className={cn('title')}>추가 참여하기</h3>
          ...
          <div className={cn('modal-body')}>
            ...
            <div className={cn('modal-body-inner')}>
              {stepCount === 1 && <StationStep2 />}
              {stepCount === 2 && <StationStep3 />}
            </div>
          </div>
          <div className={cn('modal-footer')}>
            {stepCount === 2 && <OutlineButton buttonText="이전" size="md" color="black" onClick={() => stepDown()} />}
            {stepCount === 1 ? (
              <BgButton
                buttonText="다음"
                size="md"
...
export default DaoStationAddModal;
